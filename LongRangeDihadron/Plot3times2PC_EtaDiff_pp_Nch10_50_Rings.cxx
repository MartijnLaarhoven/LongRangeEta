...
void Plot3times2PC_EtaDiff_pp_Nch10_50_Rings() {
    ...
    TFile* fInnerC = OpenFirstExistingFile({
        ...
        "./3times2PC/Vn_LHC24af_pass1_innerRing_nch10_50_FT0C_Mult_10_50.root"
    }, pathInnerC);
    TFile* fInnerA = OpenFirstExistingFile({
        "./3times2PC/Vn_LHC24af_pass1_innerRing_nch10_50_FT0A_Nch_10_50.root",
        "./3times2PC/Vn_LHC24af_pass1_innerRing_nch10_50_FT0A_Mult_10_50.root"
    }, pathInnerA);
    TFile* fOuterC = OpenFirstExistingFile({
        "./3times2PC/Vn_LHC24af_pass1_outerRing_nch10_50_FT0C_Nch_10_50.root",
        "./3times2PC/Vn_LHC24af_pass1_outerRing_nch10_50_FT0C_Mult_10_50.root"
    }, pathOuterC);
    TFile* fOuterA = OpenFirstExistingFile({
        "./3times2PC/Vn_LHC24af_pass1_outerRing_nch10_50_FT0A_Nch_10_50.root",
        "./3times2PC/Vn_LHC24af_pass1_outerRing_nch10_50_FT0A_Mult_10_50.root"
    }, pathOuterA);

    if (!fFull) {
        std::cerr << "Cannot open pp Nch full file (tried Nch/Mult candidates)." << std::endl;
        return;
    }

    std::cout << "[Plot pp Nch rings] Full file: " << pathFull << std::endl;
    std::cout << "[Plot pp Nch rings] Inner FT0C file: " << (pathInnerC.empty() ? "<missing>" : pathInnerC) << std::endl;
    std::cout << "[Plot pp Nch rings] Inner FT0A file: " << (pathInnerA.empty() ? "<missing>" : pathInnerA) << std::endl;
    std::cout << "[Plot pp Nch rings] Outer FT0C file: " << (pathOuterC.empty() ? "<missing>" : pathOuterC) << std::endl;
    std::cout << "[Plot pp Nch rings] Outer FT0A file: " << (pathOuterA.empty() ? "<missing>" : pathOuterA) << std::endl;

    TH1D* hFull = LoadV2Combined(fFull);
    TH1D* hInnerSides = BuildSidesFromFiles(fInnerC, fInnerA, "hppInnerSidesNch");
    TH1D* hOuterSides = BuildSidesFromFiles(fOuterC, fOuterA, "hppOuterSidesNch");

    if (!hFull || !hInnerSides || !hOuterSides) {
        std::cerr << "Missing required histograms for pp Nch ring-combination plot." << std::endl;
    ...
    TGraphAsymmErrors* gFilled = BuildGraph("gppNchRingsFilled", points, false);
    TGraphAsymmErrors* gMirror = BuildGraph("gppNchRingsMirror", points, true);

    if (!gFilled) {
        std::cerr << "Failed to build pp Nch ring-combination graph." << std::endl;
        if (fFull) { fFull->Close(); delete fFull; }
        if (fInnerC) { fInnerC->Close(); delete fInnerC; }
        if (fInnerA) { fInnerA->Close(); delete fInnerA; }
        if (fOuterC) { fOuterC->Close(); delete fOuterC; }
    ...
    StyleFourSystemLike(gFilled, kBlack, 21);
    StyleFourSystemLike(gMirror, kBlack, 25);

    TH1D* hFrame = BuildEtaFrame("hFrameppNchRings");
    ...
    TCanvas* c = new TCanvas("cppNchRings", "pp 3times2PC Nch 10-50 with rings", 1150, 700);
    ...
    leg->AddEntry(gFilled, "pp (Nch 10-50)", "lep");
    leg->Draw();
    ...
    c->SaveAs("./3times2PC/Plot_v2_FT0Side_pp_Nch10_50_inner_outer_full.root");
    c->SaveAs("./3times2PC/Plot_v2_FT0Side_pp_Nch10_50_inner_outer_full.png");
