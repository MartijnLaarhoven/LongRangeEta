...
TH1D* BuildRatioHistogram(const char* name) {
    ...
    h->GetXaxis()->SetTitle("#eta");
    h->GetYaxis()->SetTitle("O-O TPCCrossedRows90 / O-O");
    h->SetStats(0);
    h->SetTitle("");
...
void Compare3times2PC_EtaDiff_TwoSystems_OO_TPCCrossedRows90_Rings() {
    gStyle->SetOptStat(0);
    ...
    const char* fileNeNeFull    = "./3times2PC/Vn_LHC25ae_pass2_659120_TPCCrossedRows90_Cent_0_20.root";
    const char* fileNeNeInnerC  = "./3times2PC/Vn_LHC25ae_pass2_innerRing_FT0C_TPCCrossedRows90_Cent_0_20.root";
    const char* fileNeNeInnerA  = "./3times2PC/Vn_LHC25ae_pass2_innerRing_FT0A_TPCCrossedRows90_Cent_0_20.root";
    const char* fileNeNeOuterC  = "./3times2PC/Vn_LHC25ae_pass2_outerRing_FT0C_TPCCrossedRows90_Cent_0_20.root";
    const char* fileNeNeOuterA  = "./3times2PC/Vn_LHC25ae_pass2_outerRing_FT0A_TPCCrossedRows90_Cent_0_20.root";
    ...
    std::cout << "[Compare rings] O-O TPCCrossedRows90 inputs: full, inner FT0C, inner FT0A, outer FT0C, outer FT0A" << std::endl;
    ...
    if (!fNeNeFull || !fNeNeFull->IsOpen()) {
        std::cerr << "Cannot open O-O TPCCrossedRows90 full-range file: " << fileNeNeFull << std::endl;
        fOOFull->Close();
        delete fOOFull;
        return;
    }
    ...
    if (!fNeNeInnerC || !fNeNeInnerC->IsOpen()) {
        std::cerr << "Warning: Cannot open O-O TPCCrossedRows90 inner-ring FT0C file: " << fileNeNeInnerC << std::endl;
    }
    if (!fNeNeInnerA || !fNeNeInnerA->IsOpen()) {
        std::cerr << "Warning: Cannot open O-O TPCCrossedRows90 inner-ring FT0A file: " << fileNeNeInnerA << std::endl;
    }
    if (!fNeNeOuterC || !fNeNeOuterC->IsOpen()) {
        std::cerr << "Warning: Cannot open O-O TPCCrossedRows90 outer-ring FT0C file: " << fileNeNeOuterC << std::endl;
    }
    if (!fNeNeOuterA || !fNeNeOuterA->IsOpen()) {
        std::cerr << "Warning: Cannot open O-O TPCCrossedRows90 outer-ring FT0A file: " << fileNeNeOuterA << std::endl;
    }
    ...
    for (const int harmonic : harmonics) {
        ...
        TH1D* hNeNeInner = BuildRingSidesFromSideFiles(fNeNeInnerC, fNeNeInnerA, harmonic, Form("hNeNeInnerSides_v%d", harmonic), Form("O-O TPCCrossedRows90 inner sides;side;v_{%d}", harmonic));
        TH1D* hNeNeOuter = BuildRingSidesFromSideFiles(fNeNeOuterC, fNeNeOuterA, harmonic, Form("hNeNeOuterSides_v%d", harmonic), Form("O-O TPCCrossedRows90 outer sides;side;v_{%d}", harmonic));
        ...
        if (!hOOFull || !hNeNeFull) {
            std::cout << "[Compare rings] Skip v" << harmonic
                      << ": missing full-range histogram in O-O or O-O TPCCrossedRows90 file." << std::endl;
            continue;
        }
        ...
        leg->AddEntry(gNeNeFilled,  "O-O TPCCrossedRows90", "lep");
        ...
        c->SaveAs(Form("./3times2PC/Compare_v%d_FT0Side_OO_vs_OO_TPCCrossedRows90_inner_outer_full.root", harmonic));
        c->SaveAs(Form("./3times2PC/Compare_v%d_FT0Side_OO_vs_OO_TPCCrossedRows90_inner_outer_full.png", harmonic));
        ...
        if (gOOFullOnlyFilled && gNeNeFullOnlyFilled && gFullOnlyRatioFilled) {
            ...
            if (hFullOnlyFrame && hFullOnlyRatioFrame) {
                ...
                legFullOnly->AddEntry(gNeNeFullOnlyFilled, "O-O TPCCrossedRows90 (full FT0)", "lep");
                ...
                cFullOnly->SaveAs(Form("./3times2PC/Compare_v%d_FT0Full_OO_vs_OO_TPCCrossedRows90_ratio.root", harmonic));
                cFullOnly->SaveAs(Form("./3times2PC/Compare_v%d_FT0Full_OO_vs_OO_TPCCrossedRows90_ratio.png", harmonic));
                std::cout << "[Compare rings] Saved full-FT0-only O-O vs O-O TPCCrossedRows90 comparison for v"
                          << harmonic << " as Compare_v" << harmonic
                          << "_FT0Full_OO_vs_OO_TPCCrossedRows90_ratio.png" << std::endl;
            }
        }
        ...
        DrawSingleSystem(harmonic,
                         ...
                         Form("3x2PC O-O TPCCrossedRows90 only v%d", harmonic),
                         Form("Compare_v%d_FT0Side_OO_TPCCrossedRows90_only_inner_outer_full", harmonic),
                         gNeNeFilled,
                         gNeNeMirror,
                         "O-O TPCCrossedRows90",
                         "O-O TPCCrossedRows90, FT0 rings + TPC acceptance",
                         maxVal);
        ...
        DrawRingToFullSystem(harmonic,
                             ...
                             Form("FT0 ring/full check O-O TPCCrossedRows90 v%d", harmonic),
                             Form("Compare_v%d_FT0RingToFull_OO_TPCCrossedRows90", harmonic),
                             "O-O TPCCrossedRows90",
                             kGreen + 2,
                             hNeNeFull,
                             hNeNeInner,
        ...
        if (!ooHasRings) {
            ...
            std::cout << "Saved v" << harmonic
                      << " systematic-check comparison plot with FT0 ring points for O-O and O-O TPCCrossedRows90." << std::endl;
        }
            std::cout << "Saved individual O-O and O-O TPCCrossedRows90 v" << harmonic << " plots in the same style." << std::endl;
    }
