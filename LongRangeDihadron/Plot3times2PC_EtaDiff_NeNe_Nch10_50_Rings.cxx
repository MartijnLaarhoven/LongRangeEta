...
void Plot3times2PC_EtaDiff_NeNe_Nch10_50_Rings() {
    ...
    TFile* fInnerC = OpenFirstExistingFile({
        ...
        "./3times2PC/Vn_LHC25af_pass2_innerRing_nch10_50_FT0C_Mult_10_50.root"
    }, pathInnerC);
    TFile* fInnerA = OpenFirstExistingFile({
        "./3times2PC/Vn_LHC25af_pass2_innerRing_nch10_50_FT0A_Nch_10_50.root",
        "./3times2PC/Vn_LHC25af_pass2_innerRing_nch10_50_FT0A_Mult_10_50.root"
    }, pathInnerA);
    TFile* fOuterC = OpenFirstExistingFile({
        "./3times2PC/Vn_LHC25af_pass2_outerRing_nch10_50_FT0C_Nch_10_50.root",
        "./3times2PC/Vn_LHC25af_pass2_outerRing_nch10_50_FT0C_Mult_10_50.root"
    }, pathOuterC);
    TFile* fOuterA = OpenFirstExistingFile({
        "./3times2PC/Vn_LHC25af_pass2_outerRing_nch10_50_FT0A_Nch_10_50.root",
        "./3times2PC/Vn_LHC25af_pass2_outerRing_nch10_50_FT0A_Mult_10_50.root"
    }, pathOuterA);

    if (!fFull) {
        std::cerr << "Cannot open Ne-Ne Nch full file (tried Nch/Mult candidates)." << std::endl;
        return;
    }

    std::cout << "[Plot Ne-Ne Nch rings] Full file: " << pathFull << std::endl;
    std::cout << "[Plot Ne-Ne Nch rings] Inner FT0C file: " << (pathInnerC.empty() ? "<missing>" : pathInnerC) << std::endl;
    std::cout << "[Plot Ne-Ne Nch rings] Inner FT0A file: " << (pathInnerA.empty() ? "<missing>" : pathInnerA) << std::endl;
    std::cout << "[Plot Ne-Ne Nch rings] Outer FT0C file: " << (pathOuterC.empty() ? "<missing>" : pathOuterC) << std::endl;
    std::cout << "[Plot Ne-Ne Nch rings] Outer FT0A file: " << (pathOuterA.empty() ? "<missing>" : pathOuterA) << std::endl;

    TH1D* hFull = LoadV2Combined(fFull);
    TH1D* hInnerSides = BuildSidesFromFiles(fInnerC, fInnerA, "hNeNeInnerSidesNch");
    TH1D* hOuterSides = BuildSidesFromFiles(fOuterC, fOuterA, "hNeNeOuterSidesNch");

    if (!hFull || !hInnerSides || !hOuterSides) {
        std::cerr << "Missing required histograms for Ne-Ne Nch ring-combination plot." << std::endl;
    ...
    TGraphAsymmErrors* gFilled = BuildGraph("gNeNeNchRingsFilled", points, false);
    TGraphAsymmErrors* gMirror = BuildGraph("gNeNeNchRingsMirror", points, true);

    if (!gFilled) {
        std::cerr << "Failed to build Ne-Ne Nch ring-combination graph." << std::endl;
        if (fFull) { fFull->Close(); delete fFull; }
        if (fInnerC) { fInnerC->Close(); delete fInnerC; }
        if (fInnerA) { fInnerA->Close(); delete fInnerA; }
        if (fOuterC) { fOuterC->Close(); delete fOuterC; }
    ...
    TH1D* hFrame = BuildEtaFrame("hFrameNeNeNchRings");
    ...
    TCanvas* c = new TCanvas("cNeNeNchRings", "Ne-Ne 3times2PC Nch 10-50 with rings", 1150, 700);
    ...
    c->SaveAs("./3times2PC/Plot_v2_FT0Side_NeNe_Nch10_50_inner_outer_full.root");
    c->SaveAs("./3times2PC/Plot_v2_FT0Side_NeNe_Nch10_50_inner_outer_full.png");
