...
struct InputUnit {
    ...
    InputUnit(std::string _fileNameSuffix, Bool_t _isNch, Bool_t _isPtdiff, Int_t _minRange, Int_t _maxRange) :
        fileNameSuffix(_fileNameSuffix), isNch(_isNch), isPtdiff(_isPtdiff), minRange(_minRange), maxRange(_maxRange) {}
};
    ...
    for (auto input : inputList) {
        if (input.isPtdiff) {
            ...
            for (int iPt = 0; iPt < pTBins.size() - 1; iPt++) {
                ...
                CreateBootstrapSample_PtDiff(input.fileNameSuffix, input.isNch, input.minRange, input.maxRange, pTMin, pTMax);
            }
        } else {
