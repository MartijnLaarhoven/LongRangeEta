...
void Process_TemplateFit() {
    ...
    configList.push_back(ConfigUnit(kCent, kPtDiffOn, InputUnit("LHC25af_cpass0_QC1_449371", 80, 100), 
    {InputUnit("LHC25af_cpass0_QC1_448956", 0, 20)}, 
    "LHC25af_cpass0_QC1_448956"));
...
void ProcessConfig_PtDiff(Bool_t isNch, InputUnit templ, std::vector<InputUnit> dataList, std::string outputFileName) {
    ...
    for (const auto& data : dataList) {
        ...
        std::string splitName = "Mult";
        if (!isNch) splitName = "Cent";
        TFile outputFile(Form("./TemplateFit/PtDiff/Vn_%s_%s_%i_%i.root", outputFileName.c_str(), splitName.c_str(), data.minRange, data.maxRange), "RECREATE");
        ...
        std::cout << "Output file: " << Form("./TemplateFit/PtDiff/Vn_%s_%s_%i_%i.root", outputFileName.c_str(), splitName.c_str(), data.minRange, data.maxRange) << std::endl;
        outputFile.Close();
...
VnUnit* TemplateFit(Bool_t isNch, InputUnit templ, InputUnit data, Bool_t cn2Tovn2, Double_t pTMin=0, Double_t pTMax=0) {
    ...
    if (pTMin > 0 && pTMax > 0) {
        templatefile_PtDiff = new TFile(Form("./ProcessOutput/PtDiff/BootstrapSample_%s_%s_%d_%d_Pt_%0.1f_%0.1f.root", templ.fileNameSuffix.c_str(), splitName.c_str(), templ.minRange, templ.maxRange, pTMin, pTMax), "READ");
        if (!templatefile_PtDiff || !templatefile_PtDiff->IsOpen()) {
            std::cerr << "Cannot open template file: " << Form("./ProcessOutput/PtDiff/BootstrapSample_%s_%s_%d_%d_Pt_%0.1f_%0.1f.root", templ.fileNameSuffix.c_str(), splitName.c_str(), templ.minRange, templ.maxRange, pTMin, pTMax) << std::endl;
            exit(1);
        }
    }
    ...
    if (pTMin > 0 && pTMax > 0) {
        ...
        if (!datafile_PtDiff || !datafile_PtDiff->IsOpen()) {
            std::cerr << "Cannot open input file: " << Form("./ProcessOutput/PtDiff/BootstrapSample_%s_%s_%i_%i_Pt_%0.1f_%0.1f.root", data.fileNameSuffix.c_str(), splitName.c_str(), data.minRange, data.maxRange, pTMin, pTMax) << std::endl;
            exit(1);
        }
    }
