...
struct InputUnit {
    ...
    InputUnit(std::string _fileNameSuffix, Bool_t _isNch, Bool_t _isPtdiff, Int_t _minRange, Int_t _maxRange, Bool_t _isMc=false) :
        fileNameSuffix(_fileNameSuffix), isNch(_isNch), isPtdiff(_isPtdiff), minRange(_minRange), maxRange(_maxRange), isMc(_isMc) {}
};
...
void Read_dPhidEta_givenRange_PtDiff(std::string fileNameSuffix, Bool_t isNch, Int_t minRange, Int_t maxRange, Double_t pTMin, Double_t pTMax, Bool_t isMc);
...
void Process_dPhidEta() {
    ...
    inputList.push_back(InputUnit("LHC25af_pass1_468151", kCent, kPtDiffOff, 0, 5));    
    inputList.push_back(InputUnit("LHC25af_pass1_468151", kCent, kPtDiffOn, 0, 5));    
    ...
    for (auto input : inputList) {
        if (input.isPtdiff) {
            for (int iPt = 0; iPt < pTBins.size() - 1; iPt++) {
                ...
                Read_dPhidEta_givenRange_PtDiff(input.fileNameSuffix, input.isNch, input.minRange, input.maxRange, pTMin, pTMax, input.isMc);
            }
        }
        else {
...
void Read_dPhidEta_givenRange(std::string fileNameSuffix, Bool_t isNch, Int_t minRange, Int_t maxRange, Bool_t isMc=false) {
    ...
    if (isMc) {
        if (!file->Get(Form("di-hadron-cor_%s%s_%d_%d/MCTrue", additionalSuffix.c_str(), splitName.c_str(), minRange, maxRange))) {
    ...
    CorrelationContainer *same = (CorrelationContainer*)file->Get(Form("di-hadron-cor_%s%s_%d_%d/sameEvent", additionalSuffix.c_str(), splitName.c_str(), minRange, maxRange));
    CorrelationContainer *mixed = (CorrelationContainer*)file->Get(Form("di-hadron-cor_%s%s_%d_%d/mixedEvent", additionalSuffix.c_str(), splitName.c_str(), minRange, maxRange));
    THnSparseD *trig = nullptr;
    if (!isMc) {
        trig = (THnSparseD*)file->Get(Form("di-hadron-cor_%s%s_%d_%d/Trig_hist", additionalSuffix.c_str(), splitName.c_str(), minRange, maxRange));
    } else {
        trig = (THnSparseD*)file->Get(Form("di-hadron-cor_%s%s_%d_%d/MCTrue/MCTrig_hist", additionalSuffix.c_str(), splitName.c_str(), minRange, maxRange));
    }
...
void Read_dPhidEta_givenRange_PtDiff(std::string fileNameSuffix, Bool_t isNch, Int_t minRange, Int_t maxRange, Double_t pTMin, Double_t pTMax, Bool_t isMc=false) {
    TFile *file = TFile::Open(Form("../AnalysisResultsROOTFiles/dihadronCorr/AnalysisResults_%s.root", fileNameSuffix.c_str()), "READ");
    if (!file || file->IsZombie()) {
        std::cout << "Error: Cannot open file " << fileNameSuffix << std::endl;
        return;
    ...
    // Create output file
    TFile* fout = TFile::Open(Form("./ProcessOutput/PtDiff/Mixed_%s%s_%s_%i_%i_Pt_%0.1f_%0.1f.root", fileNameSuffix.c_str(), additionalSuffix.c_str(), splitName.c_str(), int(minRange), int(maxRange), pTMin, pTMax), "RECREATE");
    ...
    delete sparMix;
    // End of process
    std::cout << "Output file: " << Form("./ProcessOutput/PtDiff/Mixed_%s%s_%s_%i_%i_Pt_%0.1f_%0.1f.root", fileNameSuffix.c_str(), additionalSuffix.c_str(), splitName.c_str(), int(minRange), int(maxRange), pTMin, pTMax) << std::endl;
    std::cout << "Processing completed for all samples." << std::endl;
}
